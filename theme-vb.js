...
function getAuthorImg(entry){try{var img=entry.author[0]["gd$image"];if(img&&img.src&&img.src.indexOf("silhouette")===-1&&img.src.indexOf("placeholder")===-1){return img.src;}}catch(e){}
return null;}
function getAuthorUri(entry){try{return entry.author[0].uri.$t||"";}catch(e){return"";}}
function tsOf(iso){if(!iso)return 0;var t=Date.parse(iso);return isNaN(t)?0:t;}
function buildAvatarCache(entries){entries.forEach(function(e){var name=getAuthorName(e);if(_avatarCache[name])return;var img=getAuthorImg(e);if(img)_avatarCache[name]=img;});}
function applyAvatarCache(){document.querySelectorAll("[data-avatar-author]").forEach(function(el){var name=el.getAttribute("data-avatar-author");if(!name||!_avatarCache[name])return;el.innerHTML='<img src="'+_avatarCache[name]
+'" alt="'+name
+'" loading="lazy" style="width:100%;height:100%;object-fit:cover;">';});}
var _fCache={};var _fTTL=900000;function cachedFetch(url){var now=Date.now();var c=_fCache[url];if(c&&(now-c.ts)<_fTTL)return Promise.resolve(c.data);return fetch(url).then(function(r){return r.ok?r.json():null;}).catch(function(){return null;}).then(function(d){_fCache[url]={ts:now,data:d};return d;});}function loadPageAvatars(){if(!document.querySelector("[data-avatar-author]"))return;cachedFetch("/feeds/posts/summary?alt=json&max-results=500").then(function(data){buildAvatarCache((data&&data.feed&&data.feed.entry)||[]);applyAvatarCache();});}
function vbGetToken(){try{var exp=parseInt(localStorage.getItem(VB_TOKEN_EXP)||"0",10);if(exp&&Date.now()>exp){localStorage.removeItem(VB_TOKEN_KEY);localStorage.removeItem(VB_TOKEN_EXP);return null;}
return localStorage.getItem(VB_TOKEN_KEY)||null;}catch(e){return null;}}
function vbSaveToken(tk,expiresIn){try{var exp=Date.now()+((parseInt(expiresIn,10)||3600)-60)*1000;localStorage.setItem(VB_TOKEN_KEY,tk);localStorage.setItem(VB_TOKEN_EXP,String(exp));}catch(e){}}
function vbClearToken(){try{localStorage.removeItem(VB_TOKEN_KEY);localStorage.removeItem(VB_TOKEN_EXP);}catch(e){}}
function vbLoadUserProfile(token){var VBU="vbUser3";var saved=null;try{saved=JSON.parse(localStorage.getItem(VBU));}catch(e){}function applyProfile(name,picture){window._vbUserName=name;var nameEl=document.getElementById("mifrm-user-name");var avatarEl=document.getElementById("mifrm-user-avatar");if(nameEl)nameEl.textContent=name||"Kullanıcı";if(avatarEl){if(picture){avatarEl.innerHTML='<img src="'+picture+'" alt="" loading="lazy" style="width:100%;height:100%;object-fit:cover;border-radius:50%;display:block;">';}else{avatarEl.innerHTML='<i aria-hidden="true" class="fas fa-user"></i>';}}var panel=document.getElementById("mifrm-user-panel");if(panel)panel.classList.add("active");}if(saved&&saved.name){applyProfile(saved.name,saved.picture||null);return;}fetch("https://www.googleapis.com/oauth2/v3/userinfo",{headers:{"Authorization":"Bearer "+token}}).then(function(r){return r.ok?r.json():null;}).catch(function(){return null;}).then(function(d){if(!d)return;var name=d.name||d.email||"Kullanıcı";var picture=d.picture||null;try{localStorage.setItem(VBU,JSON.stringify({name:name,picture:picture}));}catch(e){}applyProfile(name,picture);});}
function vbToast(title,desc){var box=document.getElementById("forumLiveNotification");if(!box)return;var t=document.getElementById("notifyTitle");var d=document.getElementById("notifyDesc");if(t)t.textContent=title||"";if(d)d.textContent=desc||"";box.style.display="flex";clearTimeout(box._vbt);box._vbt=setTimeout(function(){box.style.display="none";},6000);}
function vbCloseToast(){var box=document.getElementById("forumLiveNotification");if(box){clearTimeout(box._vbt);box.style.display="none";}}
...
function vbRenderNotifs(){var bellEl=document.getElementById("vbNotif");var dotEl=document.getElementById("vbNotifDot");var listEl=document.getElementById("vbNotifList");if(!bellEl)return;var state=vbGetState();var items=vbCleanExpired(state.items||[]);var dismissed=!!state.dismissed;var unread=dismissed?0:items.length;bellEl.classList.toggle("has-unread",unread>0);if(dotEl)dotEl.textContent=unread>0?(unread>9?"9+":String(unread)):"";if(!listEl)return;if(!items.length||dismissed){listEl.innerHTML='<div class="vb-notif-empty"><i class="fas fa-check-circle"></i><br/>Yeni bildirim yok.</div>';return;}
var html="";items.forEach(function(it){var isReply=it.kind==="reply";var kCls=isReply?" reply":"";var kIco=isReply?"fa-reply":"fa-pen";var kLbl=isReply?"YENİ CEVAP":"YENİ KONU";html+='<a class="vb-notif-item" href="'+(it.link||"/")+'">'
+'<span class="vb-notif-kind'+kCls+'">'
+'<i class="fas '+kIco+'" aria-hidden="true"></i> '
+kLbl+'</span>'
+'<span class="vb-notif-title">'+(it.title||"—")+'</span>'
+'<span class="vb-notif-sub">'+(it.sub||"")+'</span>'
+'</a>';});listEl.innerHTML=html;}
function vbToggleNotif(){var bellEl=document.getElementById("vbNotif");var btnEl=document.getElementById("vbNotifBell");if(!bellEl)return;var isOpen=bellEl.classList.toggle("open");if(btnEl)btnEl.setAttribute("aria-expanded",isOpen?"true":"false");}
function vbDismissAll(){var state=vbGetState();state.dismissed=true;vbSetState(state);vbRenderNotifs();var bellEl=document.getElementById("vbNotif");var btnEl=document.getElementById("vbNotifBell");if(bellEl)bellEl.classList.remove("open");if(btnEl)btnEl.setAttribute("aria-expanded","false");}
function vbCheckActivity(){var p1=cachedFetch("/feeds/posts/summary?alt=json&orderby=published&max-results="+MFG("notif.topicsCount",20));var p2=cachedFetch("/feeds/comments/default?alt=json&max-results="+MFG("notif.repliesCount",20));Promise.all([p1,p2]).then(function(res){var posts=(res[0]&&res[0].feed&&res[0].feed.entry)||[];var comments=(res[1]&&res[1].feed&&res[1].feed.entry)||[];var state=vbGetState();if(!state.seenTopic&&!state.seenComment){state.seenTopic=posts.length?posts[0].published.$t:"";state.seenComment=comments.length?comments[0].published.$t:"";state.items=[];state.dismissed=false;vbSetState(state);vbRenderNotifs();return;}
var existing=vbCleanExpired(state.items||[]);var existingIds=existing.map(function(it){return it.id;});var newItems=[];posts.forEach(function(p){var pub=tsOf(p.published.$t),seen=tsOf(state.seenTopic||"");if(seen&&pub>seen){var link=getAltLink(p);var id="t_"+(p.id?p.id.$t:pub);if(existingIds.indexOf(id)===-1){newItems.push({id:id,kind:"topic",ts:Date.now(),title:safeText(p.title?p.title.$t:"Yeni Konu"),sub:"Yazan: "+safeText(getAuthorName(p))+" · "+formatDate(p.published.$t),link:link?link.href:"/"});}}});comments.forEach(function(c){var pub=tsOf(c.published.$t),seen=tsOf(state.seenComment||"");if(seen&&pub>seen){var link=getAltLink(c);var id="c_"+(c.id?c.id.$t:pub);if(existingIds.indexOf(id)===-1){var postTitle=(c.title&&c.title.$t)?c.title.$t:(link&&link.title?link.title:"Bir konu");newItems.push({id:id,kind:"reply",ts:Date.now(),title:safeText(postTitle),sub:safeText(getAuthorName(c))+" cevap yazdı · "+formatDate(c.published.$t),link:link?link.href:"/"});}}});if(posts.length)state.seenTopic=posts[0].published.$t;if(comments.length)state.seenComment=comments[0].published.$t;state.items=newItems.concat(existing).slice(0,MFG("notif.maxItems",50));if(newItems.length){state.dismissed=false;var first=newItems[0];vbToast(first.kind==="reply"?"Yeni Cevap Geldi!":"Yeni Konu Açıldı!",first.title.replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&quot;/g,'"'));}
vbSetState(state);vbRenderNotifs();});}
function renderVbTopics(shown){var target=document.getElementById("vb-last-posts-target");if(!target||!_vbAllPosts.length)return;var slice=_vbAllPosts.slice(0,shown);var html="";slice.forEach(function(p){var avatarHtml=p.avatar?'<img src="'+p.avatar+'" alt="'+safeText(p.author)+'" loading="lazy" width="32" height="32"/>':'<i class="fas fa-user" aria-hidden="true"></i>';var catHtml=p.category?'<span class="vb-cat-badge"><i class="fas '+getCategoryIcon(p.category)+'" aria-hidden="true"></i>'+safeText(p.category)+'</span>':'';html+='<div class="hamdi-forum-row">'
...
+'</div>'
+'<div class="hamdi-replies-zone"><i class="fas fa-comment" aria-hidden="true" style="margin-right:3px"></i>'+p.replies+'</div>'
+'<div class="hamdi-arrow-zone" aria-hidden="true"><i class="fas fa-chevron-right"></i></div>'
+'</div>';});target.innerHTML=html;var moreWrap=document.getElementById("vb-topics-more");if(moreWrap)moreWrap.style.display=(shown<_vbAllPosts.length)?"block":"none";}
function loadMoreTopics(){var prev=_vbShown;_vbShown=Math.min(_vbShown+MFG("home.loadMoreStep",10),_vbAllPosts.length);renderVbTopics(_vbShown);var target=document.getElementById("vb-last-posts-target");if(target){var rows=target.querySelectorAll(".hamdi-forum-row");var nextRow=rows[prev];if(nextRow&&nextRow.scrollIntoView){nextRow.scrollIntoView({behavior:"smooth",block:"start"});}else{target.scrollTo({top:target.scrollHeight,behavior:"smooth"});}}}
function fetchBloggerViews(){var viewsEl=document.getElementById("stat-total-views");if(!viewsEl)return;function tryRead(){var span=document.getElementById("Stats1_totalCount");if(!span)return false;var raw=(span.textContent||span.innerText||"").trim();var num=raw.replace(/[^0-9]/g,"");if(num.length>0){viewsEl.textContent=Number(num).toLocaleString("tr-TR");return true;}
return false;}
if(tryRead())return;var span=document.getElementById("Stats1_totalCount");var mo=null;if(span&&typeof MutationObserver!=="undefined"){mo=new MutationObserver(function(){if(tryRead()){mo.disconnect();clearInterval(iv);}});mo.observe(span,{childList:true,characterData:true,subtree:true});}
var att=0;var iv=setInterval(function(){att++;if(tryRead()){clearInterval(iv);if(mo)mo.disconnect();return;}
if(att>=40){clearInterval(iv);if(mo)mo.disconnect();if(!viewsEl.textContent||viewsEl.textContent==="—"){viewsEl.textContent="—";}}},300);}
function fetchBloggerViewsWeek(){var onlineEl=document.getElementById("stat-online-count");if(!onlineEl)return;Promise.all([cachedFetch("/feeds/posts/summary?alt=json&max-results=500"),cachedFetch("/feeds/comments/default?alt=json&max-results=500")]).then(function(res){var authors={};((res[0]&&res[0].feed&&res[0].feed.entry)||[]).forEach(function(e){var n=getAuthorName(e);if(n&&n!=="Misafir")authors[n]=1;});((res[1]&&res[1].feed&&res[1].feed.entry)||[]).forEach(function(e){var n=getAuthorName(e);if(n&&n!=="Misafir")authors[n]=1;});var c=Object.keys(authors).length;onlineEl.textContent=c.toLocaleString("tr-TR")+" Üye";}).catch(function(){onlineEl.textContent="—";});}
function loadPostbitStats(){var joinSpan=document.querySelector(".pb-join-val");var topicsSpan=document.querySelector(".pb-topics-val");var msgsSpan=document.querySelector(".pb-msgs-val");var citySpan=document.querySelector(".pb-city-val");var followSpan=document.querySelector(".pb-follow-val");if(!joinSpan&&!topicsSpan&&!msgsSpan)return;var authorNameEl=document.querySelector(".xf-uname span[itemprop='name']");var authorName=authorNameEl?authorNameEl.textContent.trim():"";if(!authorName||authorName==="Misafir"){if(joinSpan)joinSpan.textContent="Misafir";if(topicsSpan)topicsSpan.textContent="—";if(msgsSpan)msgsSpan.textContent="—";return;}
cachedFetch("/feeds/posts/summary?alt=json&max-results=500").then(function(data){var entries=(data&&data.feed&&data.feed.entry)||[];var oldest=null;var topics=0;var totalMsgs=0;var profileUri="";entries.forEach(function(e){var name="";try{name=cleanAuthor(e.author[0].name.$t);}catch(ex){}
if(name!==authorName)return;topics++;var pub=e.published?e.published.$t:null;if(pub&&(!oldest||tsOf(pub)<tsOf(oldest)))oldest=pub;var cmt=0;try{cmt=parseInt(e["thr$total"].$t,10)||0;}catch(ex){}
totalMsgs+=cmt;if(!profileUri){try{profileUri=e.author[0].uri.$t;}catch(ex){}}});if(joinSpan)joinSpan.textContent=oldest?formatDateShort(oldest):"—";if(topicsSpan)topicsSpan.textContent=topics>0?topics:"—";if(msgsSpan)msgsSpan.textContent=totalMsgs;buildAvatarCache(entries);applyAvatarCache();if(profileUri&&citySpan){loadProfileLocation(profileUri,citySpan,followSpan);}});}
function loadProfileLocation(profileUri,cityEl,followEl){var m=profileUri.match(/\/profile\/(\d+)/);if(!m)return;var uid=m[1];fetch("https://www.blogger.com/feeds/"+uid+"/blogs?alt=json&max-results=0").then(function(r){return r.ok?r.json():null;}).catch(function(){return null;}).then(function(d){if(!d||!d.feed)return;var total=0;try{total=parseInt(d.feed["openSearch$totalResults"].$t,10);}catch(ex){}
if(followEl&&total>0)followEl.textContent=total;var loc="";try{loc=d.feed.author[0]["gd$extendedProperty"].find(function(p){return p.name==="user.location";}).value;}catch(ex){}
if(cityEl&&loc)cityEl.textContent=loc;});}
function mifrmFillCatLastPost(label,lBox,iconEl){var u="/feeds/posts/summary/-/"+encodeURIComponent(label)+"?alt=json&orderby=published&max-results=1";cachedFetch(u).then(function(d){var ents=(d&&d.feed&&d.feed.entry)||[];if(!ents.length){lBox.innerHTML='<em style="color:var(--meta);">Bu kategoride henüz konu yok.</em>';return;}var e=ents[0];var link=getAltLink(e);var isNew=e.published&&(Date.now()-tsOf(e.published.$t))<259200000;if(iconEl)iconEl.className='fas '+(isNew?'fa-folder-open':'fa-folder');lBox.innerHTML='Son Konu: <a class="cat-last-title" href="'+(link?link.href:"#")+'">'+safeText(e.title?e.title.$t:"")+'</a>'+'<div class="cat-last-meta">Gönderen: <strong>'+safeText(getAuthorName(e))+'</strong> — '+formatDate(e.published?e.published.$t:"")+'</div>';});}
function loadForumSystem(){var target=document.getElementById("vb-last-posts-target");if(!target)return;var p1=cachedFetch("/feeds/posts/summary?alt=json&max-results=500");var p2=cachedFetch("/feeds/comments/default?alt=json&max-results=1");Promise.all([p1,p2]).then(function(res){var feed=res[0]&&res[0].feed;var cmtFeed=res[1]&&res[1].feed;var entries=(feed&&feed.entry)||[];var total=feed&&feed["openSearch$totalResults"]?parseInt(feed["openSearch$totalResults"].$t,10):entries.length;var postCountEl=document.getElementById("stat-total-posts");if(postCountEl)postCountEl.textContent=total.toLocaleString("tr-TR");fetchBloggerViewsWeek();if(!entries.length){target.innerHTML='<div style="padding:24px;text-align:center;color:var(--meta);">'
+'<i class="fas fa-inbox" style="font-size:22px;display:block;margin-bottom:7px" aria-hidden="true"></i>'
+'Henüz forum konusu yok.</div>';}else{var catMap={};_vbAllPosts=[];entries.forEach(function(e){var link=getAltLink(e);var author=getAuthorName(e);var avatar=getAuthorImg(e);var item={title:safeText(e.title?e.title.$t:""),link:link?link.href:"#",author:author,avatar:avatar,date:formatDate(e.published?e.published.$t:""),published:e.published?e.published.$t:"",replies:e["thr$total"]?String(e["thr$total"].$t):"0",category:(e.category&&e.category.length)?e.category[0].term:""};_vbAllPosts.push(item);if(e.category){e.category.forEach(function(cat){if(!catMap[cat.term])catMap[cat.term]=item;});}});var catSel=document.getElementById("mifrm-category");if(catSel){var allCats={};Object.keys(catMap).forEach(function(k){allCats[k]=1;});if(feed&&feed.category)feed.category.forEach(function(c){if(c.term)allCats[c.term]=1;});var opts='<option value="">Kategori Seçin</option>';Object.keys(allCats).sort(function(a,b){return a.localeCompare(b,"tr");}).forEach(function(n){opts+='<option value="'+safeText(n)+'">'+safeText(n)+'</option>';});catSel.innerHTML=opts;}
...
+'<a href="'+t.commentLink+'">'+safeText(title)+'</a>'
+'</div>'
+'<div class="hamdi-meta">'+catHtml
+'<i class="fas fa-reply" aria-hidden="true" style="margin-right:3px;font-size:10px;"></i>'
+'<span class="hamdi-author">'+safeText(t.lastAuthor)+'</span>'
+' &middot; '+t.date
+'</div>'
+'</div>'
+'<div class="hamdi-replies-zone">'
+(replyCount!==""?'<i class="fas fa-comment" aria-hidden="true" style="margin-right:3px"></i>'+replyCount:'')
+'</div>'
+'<div class="hamdi-arrow-zone" aria-hidden="true"><i class="fas fa-chevron-right"></i></div>'
+'</div>';});target.innerHTML=html;var moreWrap=document.getElementById("vb-comments-more");if(moreWrap)moreWrap.style.display=(shown<_cmtTopics.length)?"block":"none";}
function loadMoreComments(){var prev=_cmtShown;_cmtShown=Math.min(_cmtShown+MFG("home.loadMoreStep",10),_cmtTopics.length);renderCommentRows(_cmtShown);var target=document.getElementById("vb-comments-target");if(target){var rows=target.querySelectorAll(".hamdi-forum-row");var nextRow=rows[prev];if(nextRow&&nextRow.scrollIntoView){nextRow.scrollIntoView({behavior:"smooth",block:"start"});}else{target.scrollTo({top:target.scrollHeight,behavior:"smooth"});}}}
function loadRecentComments(){var target=document.getElementById("vb-comments-target");if(!target)return;target.setAttribute("data-loaded","1");target.innerHTML='<div style="padding:20px;text-align:center;color:var(--meta);">'
+'<i class="fas fa-circle-notch fa-spin" style="margin-right:7px" aria-hidden="true"></i>Yükleniyor...</div>';cachedFetch("/feeds/comments/default?alt=json&max-results=60").then(function(data){_cmtAllEntries=(data&&data.feed&&data.feed.entry)||[];if(!_cmtAllEntries.length){target.innerHTML='<div style="padding:24px;text-align:center;color:var(--meta);">'
+'<i class="fas fa-comment-slash" style="font-size:22px;display:block;margin-bottom:7px" aria-hidden="true"></i>'
+'Henüz cevap yok.</div>';return;}
var seen={};_cmtTopics=[];_cmtAllEntries.forEach(function(e){var link="";try{for(var i=0;i<e.link.length;i++){if(e.link[i].rel==="alternate"){link=e.link[i].href;break;}}}catch(ex){}
...
function mifrmLogout(){MIFRM_USER=null;try{localStorage.removeItem("mifrm_user");}catch(e){}if(window.google&&google.accounts&&google.accounts.id){try{google.accounts.id.disableAutoSelect();}catch(e){}}mifrmRenderUser();}
function mifrmInitGoogleBtn(){var cid=(typeof MFG==="function"?MFG("signup.clientId",""):"");var holder=document.getElementById("mifrm-gbtn");if(!holder)return;if(!cid||cid.indexOf("YOUR_CLIENT_ID")!==-1){holder.innerHTML='<span style="color:var(--red);font-size:12px">Yonetici: Google Istemci Kimligi (Client ID) henuz ayarlanmamis.</span>';return;}if(!(window.google&&google.accounts&&google.accounts.id))return;try{google.accounts.id.initialize({client_id:cid,callback:mifrmGoogleCallback,auto_select:false});holder.innerHTML="";google.accounts.id.renderButton(holder,{theme:"filled_blue",size:"large",text:"signin_with",shape:"pill",locale:"tr",width:260});}catch(e){}}
